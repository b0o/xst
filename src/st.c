...
void csihandle(void) {
  ...
  case 't': /* report window pixel size */
    if (csiescseq.arg[0] == 14) {
      len = snprintf(buf, sizeof(buf), "\x1B[4;%i;%it;", xw.h, xw.w);
      ttywrite(buf, len);
    }
    break;
...
void iso14755(const Arg *arg) {
  ...
  while (1) {
    ...
    if (us == NULL || !us || *us == '\0' || *us == '-' || strlen(us) > 7)
      break;
...
void externalpipe(const Arg *arg) {
  ...
  case 0:
    ...
    close(to[0]);
    execvp("sh", (char *const[]){"/bin/sh", "-c", (char *)arg->v, 0});
    exit(127);
  }
...
void xinit(void) {
  ...
  if (!USE_ARGB)
    ...
    XVisualInfo tpl = {.screen = xw.scr, .depth = 32, .class = TrueColor};
  ...
  if (!USE_ARGB)
    ...
    xw.cmap =
        XCreateColormap(xw.dpy, XRootWindow(xw.dpy, xw.scr), xw.vis, None);
  xloadcols();
  ...
  xw.win = XCreateWindow(
      xw.dpy, parent, xw.l, xw.t, xw.w, xw.h, 0, xw.depth, InputOutput, xw.vis,
...
void xdrawglyphfontspecs(const XftGlyphFontSpec *specs, Glyph base, int len,
  ...
  if (base.mode & ATTR_STRUCK) {
    XftDrawRect(xw.draw, fg, winx,
                winy + xw.cyo - (stthick / 2) + (2 * (dc.font.ascent / 3)),
                width, stthick);
  }
...
void xdrawcursor(void) {
  ...
  if (IS_SET(MODE_HIDE) || (cursorshouldblink && !cursorblinkstyle &&
                            cursorblinkstate && (xw.state & WIN_FOCUSED)))
    return;
  ...
  if ((xw.state & WIN_FOCUSED) &&
    ...
    case 4: /* Steady Underline */
      XftDrawRect(xw.draw, &drawcol, borderpx + curx * xw.cw,
                  borderpx + (term.c.y + 1) * xw.ch - cursorthickness, xw.cw,
                  cursorthickness);
      break;
    case 5: /* Blinking bar */
    case 6: /* Steady bar */
      XftDrawRect(xw.draw, &drawcol, borderpx + curx * xw.cw,
                  borderpx + term.c.y * xw.ch, cursorthickness, xw.ch);
      break;
    }
  } else {
    ...
    XftDrawRect(xw.draw, &drawcol, borderpx + (curx + 1) * xw.cw - 1,
                borderpx + term.c.y * xw.ch, 1, xw.ch - 1);
    XftDrawRect(xw.draw, &drawcol, borderpx + curx * xw.cw,
...
void usage(void) {
  die("usage: %s [-aiv] [-A alpha] [-c class] [-f font] [-g geometry]"
      " [-n name]\n"
      "          [-o file] [-T title] [-t title] [-w windowid] [-x "
      "xresourcename]\n"
      "          [[-e] command [args ...]]\n"
...
void xrdb_load(void) {
  ...
  if (xrm != NULL) {
    ...
    cursorblinkstate = 1; // in case if cursor shape was changed from a blinking
                          // one to a non-blinking
...
int main(int argc, char *argv[]) {
  ...
  case 'x':
    ...
    if (strlen(opt_xresources) > 32)
      die("-x cannot exceet 32 characters\n");
    break;
  default:
    usage();
  }
